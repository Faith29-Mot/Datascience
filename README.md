...
The primary objective of this analysis was to test the **Research Hypothesis (H_a)**: The influence of soil health score on corn yield is significantly moderated by the level of drought severity. This hypothesized that the yield-stabilizing benefit of improved soil health would be most pronounced under conditions of high drought stress. Conversely, the **Null Hypothesis (H_0)** posited that there is no significant interaction effect, meaning the relationship between soil health and yield would be constant regardless of drought severity.

To test this, a **Two-Way Interaction Multiple Linear Regression (OLS)** model was employed, using Yield_Mg_ha as the response variable, Score_Mean as the predictor, and the categorical variable spei_cat (Drought Severity) as the moderator. Crucially, the model was fitted using **Heteroscedasticity-Consistent (HC3) Standard Errors** to ensure the robustness and reliability of all $p$-values and inferences. This correction was necessary after diagnostic checks confirmed non-constant variance in the model's residuals. Further diagnostic plots (Residuals vs. Fitted and Normal Q-Q Plot) confirmed that the model was otherwise well-specified.

**The regression results** conclusively confirmed a **highly statistically significant interaction effect** between the soil health score and drought severity, leading to the definitive rejection of H_0. This means the benefit of soil health is not static but dynamically shifts based on climate conditions. The analysis of **Conditional Slopes** provided the main interpretative finding: while soil health offered a positive, baseline yield benefit in Normal and Moderate drought years, this benefit was maximized and most potent during the Severe drought category. This strong moderation effect confirms that the role of soil health is critical for climate resilience, shifting from a simple yield contributor in normal years to a vital yield stabilizer during significant drought events.
